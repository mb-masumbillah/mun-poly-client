...
import { Search, Edit, Trash2, X, UploadCloud, Upload } from 'lucide-react';
import seatPlanData from '../../assets/data/manageSetPlane.json'; 

const ManageSitPlane = () => {
    const [data, setData] = useState(seatPlanData);
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this seat plan?")) {
  ...
  return (
    <div className="p-4 lg:p-8 bg-white min-h-screen font-sans">
      ...
      <div className="mb-6 text-center lg:text-left">
        <h1 className="text-2xl font-bold text-[#003049]">Manage Seat Plan</h1>
        <p className="text-sm font-semibold text-gray-500 mt-2">Upload and manage exam seat arrangements</p>
      </div>
      ...
      <div className="mb-10 border-2 border-dashed border-gray-200 rounded-xl p-6 lg:p-10 flex flex-col items-center justify-center bg-gray-50/30">
        ...
        <p className="text-gray-700 font-medium text-center">Upload new Seat Plan PDF/Image</p>
        <button 
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="flex justify-between items-center p-4 border-b">
              <span className="font-bold flex items-center gap-2 text-[#003049]"><Upload size={18}/> Upload Seat Plan</span>
            ...
            <div className="p-10 flex flex-col items-center">
               ...
               <p className="mt-3 text-xs text-gray-400 italic">Accepted: PDF, PNG, JPG</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-xl w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center p-5 border-b">
              <h3 className="font-bold text-[#003049]">Edit Seat Plan Info</h3>
              <button onClick={() => setIsEditing(false)}><X size={24} /></button>
            </div>
            <form onSubmit={handleSaveEdit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-400 mb-1">Plan Title</label>
                <input 
                  type="text" 
                  value={currentItem.title}
                  onChange={(e) => setCurrentItem({...currentItem, title: e.target.value})}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-1 focus:ring-blue-500 outline-none"
                />
              </div>
              <div>
              ...
              <button type="submit" className="w-full bg-[#004D61] text-white py-2 rounded-lg font-bold hover:bg-opacity-90 transition">
                Save Changes
              </button>
            </form>
          </div>
